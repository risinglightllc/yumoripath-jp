...
## Recent changes
- **2026-09-02** — Fixed three deploy-blocking bugs found during end-to-end QA of the 2026-08-31 subscription launch: (1) Render's build command for this service is `npm install` only (not `npm run build`), so `migrate.js`'s core `users` table was never created in production — `server.js`'s own startup migration runner now creates it too. (2) This Stripe account's API version (`2026-05-27.dahlia`) moved `current_period_end`/`start` off the top-level Subscription object onto `subscription.items.data[0]` — `routes/billing.js` now reads it from there (`currentPeriodEnd()` helper), fixing a webhook crash on every `checkout.session.completed` and `customer.subscription.updated` event. (3) Confirmed the full signup → Stripe Checkout (test mode) → webhook → `/map` unlock flow works end-to-end.
